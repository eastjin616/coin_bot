...
export default function StockTab() {
  ...
  const load = async () => {
    const [wl, sg] = await Promise.all([getWatchlist(), getSignals('stock')])
    setWatchlist(wl.filter((w: WatchItem) => w.market === 'stock' && w.active))
    setSignals(sg)
  }
  ...
  const add = async () => {
    if (!newSymbol) return
    await addWatchlist({ market: 'stock', symbol: newSymbol.trim(), name: newName.trim() })
    setNewSymbol(''); setNewName('')
    load()
  }

  const remove = async (symbol: string) => {
    await removeWatchlist('stock', symbol); load()
  }

  return (
    <div className="space-y-6">
      <div className="bg-gray-800 rounded-xl p-5">
        <h2 className="text-lg font-bold mb-3 text-green-400">📈 감시 종목 (주식)</h2>
        <div className="flex gap-2 mb-3">
          <input className="bg-gray-700 text-white rounded px-3 py-1 flex-1" placeholder="종목코드 (예: 005930.KS)" value={newSymbol} onChange={e => setNewSymbol(e.target.value)} />
          ...
          <button className="bg-green-600 hover:bg-green-500 text-white rounded px-3 py-1" onClick={add}>추가</button>
        </div>
      ...
      <div className="bg-gray-800 rounded-xl p-5">
        <h2 className="text-lg font-bold mb-3 text-green-400">최근 신호</h2>
